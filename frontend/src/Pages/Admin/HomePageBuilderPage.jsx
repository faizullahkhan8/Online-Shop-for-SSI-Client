...
import { useGetHomePage, useUpdateHomePage, useResetHomePage } from "../../api/hooks/homePage.api.js";
...
const SECTION_META = {
    ...
    mid_banners: { label: "Mid Banners", icon: "🌟", color: "bg-cyan-100 text-cyan-700", category: "Banner" },
            ...
            return (
                <div className="p-1">
                    <div className="bg-purple-50 border border-purple-100 rounded-xl p-4 text-sm text-purple-700">
                        <p className="font-semibold mb-1">🖼️ Hero Banner</p>
                        <p className="text-xs">Hero slides are managed separately via <strong>Hero Content</strong> page in the sidebar.</p>
                        <a href="/admin-dashboard/hero" target="_blank" rel="noreferrer"
                            className="inline-flex items-center gap-1 mt-2 text-xs font-bold text-purple-600 hover:text-purple-800">
                            Open Hero Manager <ExternalLink size={11} />
                        </a>
                    </div>
                </div>
            );
        ...
        case "brands":
            return (
                <div>
                    <TextField label="Section Title" value={cfg.title} onChange={v => set("title", v)} />
                    <TextField label="Subtitle" value={cfg.subtitle} onChange={v => set("subtitle", v)} />
                    <SectionDivider label="Brands" />
                    {(cfg.brands || []).map((brand, i) => (
                        <div key={i} className="flex items-center gap-2 mb-2">
                            <input value={brand.abbr} onChange={e => set("brands", cfg.brands.map((b, idx) => idx === i ? { ...b, abbr: e.target.value } : b))}
                                className="w-16 text-xs font-mono border border-gray-200 rounded-lg px-2 py-1.5 text-center" placeholder="ABT" />
                            <input value={brand.name} onChange={e => set("brands", cfg.brands.map((b, idx) => idx === i ? { ...b, name: e.target.value } : b))}
                                className="flex-1 text-sm border border-gray-200 rounded-lg px-2 py-1.5" placeholder="Brand Name" />
                            <button onClick={() => set("brands", cfg.brands.filter((_, idx) => idx !== i))} className="text-red-400 hover:text-red-600 shrink-0"><Trash2 size={13} /></button>
                        </div>
                    ))}
                    <button onClick={() => set("brands", [...(cfg.brands || []), { name: "New Brand", abbr: "NB" }])}
                        className="w-full py-2 border-2 border-dashed border-gray-200 rounded-xl text-xs font-semibold text-gray-400 hover:border-blue-300 hover:text-blue-500 transition-colors flex items-center justify-center gap-1 mt-2">
...
const MiniPreview = ({ section }) => {
    ...
    return (
        <div className="p-4 space-y-3">
            ...
            {["hero", "categories", "ribbon", "promo_banners", "mid_banners", "conditions", "app_download", "blogs", "featured_category", "products_grid"].includes(section.type) && (
